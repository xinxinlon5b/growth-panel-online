# 腾讯云 ADP 前沿部署工程师（FDE）认证课程 · 第 4 章
...
**裁判提示词**：用于对评测结果进行打分的模型提示词，支持通过快捷键引入评测集中的查询（query）、预置列、自定义列、应用的输出内容以及其他系统变量。请确保裁判模型提示词与评测集内容的匹配程度，以保证裁判模型能够正确打分。例如，若按照参考答案进行打分，则需要在评测集中上传参考答案（例如：reference_output）。同时支持点击**模板**，查看提示词模板库中更多适合的提示词。
...
| 问题现象 | 优先排查项 |
| --- | --- |
| **没命中** | 确认目标文档是否已上传、解析成功并处于可用状态 检查问题关键词、召回范围和权限过滤 查看切片是否包含可独立理解的答案信息 用测试问题逐项验证召回结果 |
| **命中旧文档** | 核对文档版本、更新时间和生效状态 下线或删除过期知识并重新索引 检查同名文档与重复切片 建立版本与发布管理流程 |
| **答案不完整** | 检查切片是否过短或割裂上下文 调整切片重叠、召回数量和相关性阈值 确认问题是否需要多段信息组合 优化 Prompt，要求覆盖关键字段 |
| **引用错误** | 对照回答与实际召回片段 检查引用字段、文档元数据和节点变量映射 避免模型生成未召回的来源 用标准问题集回归验证引用准确率 |
